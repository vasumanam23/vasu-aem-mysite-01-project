...
@Component(service =  WorkflowProcess.class, property = "process.label= Modified Referenced Asset")
public class ModifiedReferencedAssetWorkflow implements WorkflowProcess{
	
	private static final Logger LOG=LoggerFactory.getLogger(ModifiedReferencedAssetWorkflow.class);
	
	@Reference
    ModifiedReferencedAssetService modifiedAssetService;
	...
	public void execute(WorkItem workItem, WorkflowSession wfSession, MetaDataMap metaDataMap) throws WorkflowException {
		...
		if(StringUtils.isNotEmpty(path)) {
			Map<String, Asset> modifiedAsset=modifiedAssetService.getModifiedReferencedAsset(path);
			String[] processArgs=metaDataMap.get("PROCESS_ARGS","default").split("=");
			
			
			for(Map.Entry<String, Asset> entry:modifiedAsset.entrySet()) {
